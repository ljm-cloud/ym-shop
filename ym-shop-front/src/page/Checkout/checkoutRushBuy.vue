          ...
          <div class="box-inner ui-cart">
            <div>
              ...
              <div class="cart-table-title">
                ...
                <span class="price">秒杀价</span>
              </div>
              <!--列表-->
              <div class="cart-table">
                <div class="cart-group divide pr" :data-productid="rushBuyDetail.promId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      <!--图片-->
                      <div class="items-thumb fl">
                        <img :alt="rushBuyDetail.productName"
                             :src="rushBuyDetail.productImageBig">
                        <a @click="goodsDetails(rushBuyDetail.promId)" :title="rushBuyDetail.productName" target="_blank"></a>
                      </div>
                      <!--信息-->
                      <div class="name hide-row fl">
                        <div class="name-table">
                          <a @click="goodsDetails(rushBuyDetail.promId)" :title="rushBuyDetail.productName" target="_blank"
                             v-text="rushBuyDetail.productName"></a>
                          <!-- <ul class="attribute">
                            <li>白色</li>
                          </ul> -->
                        </div>
                      ...
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">¥ {{rushBuyDetail.salePrice}}</div>
                        <!--数量-->
                        <div class="item-cols-num">
                          <span v-text="1"></span>
                        </div>
                        <!--价格-->
                        <div class="price">¥ {{rushBuyDetail.salePrice}}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="shipping">
                  <div class="shipping-box" style="padding: 0 40px;">
                    <div class="shipping-total shipping-price"><h4
                      class="highlight">应付总额：<em>￥{{rushBuyDetail.salePrice}}</em>
                    </h4>
                    </div>
                  </div>
...
<script>
  import { addressList, addressUpdate, addressAdd, addressDel, productDet, submitOrder, getRushBuyDetails } from '/api/goods'
  ...
  export default {
    ...
    methods: {
      ...
      goodsDetails (id) {
        window.open(window.location.origin + '#/rushBuyDetails?promId=' + id)
      },
      _getCartList (promId) {
        // alert(promId)
        getRushBuyDetails({params: {promId}}).then(res => {
          this.rushBuyDetail = res.result
        })
      },
      _addressList () {
    ...
    created () {
      ...
      this.promId = query.promId
      this._getCartList(this.promId)
      this._addressList()
    },
    components: {
      YShelf,
