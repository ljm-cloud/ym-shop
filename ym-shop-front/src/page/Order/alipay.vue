...
<script>
  ...
  import {alipayQrCode} from '/api/goods'
  ...
  export default {
    ...
    methods: {
      ...
      _alipayQrCode (orderId) {
        alipayQrCode({params: { orderId }}).then(res => {
          if (res.success === true) {
            this.qrCodeUrl = res.result
            this.bindQRCode()
          } else {
            this.payNow = '发起支付出现异常'
            this.submit = false
          }
        })
      },
      bindQRCode: function () {
        let code = new QRCode(this.$refs.qrCodeDiv, {
          text: this.qrCodeUrl,
          width: 180,
          height: 180,
          colorDark: '#333333',
    ...
    mounted () {
      ...
      if (isCustom !== 'true') {
        ...
        this._alipayQrCode(this.orderId)
      }
    },
    components: {
      YShelf,
