...
//        表单传userId的方式不安全，改成通过cookies的sn token认证后获取当前用户信息
        cart.setOrderSite(OrderSiteEnum.main.name());
        int result=cartService.addCart(MemberUtils.getUserId(),cart.getProductId(),
                cart.getProductNum(),cart.getOrderSite(),cart.getPromId());
    ...
    public Result<List<CartProduct>> getCartList(@RequestBody Cart cart){
        ...
        List<CartProduct> list=cartService.getCartList(MemberUtils.getUserId(),cart.getOrderSite());
