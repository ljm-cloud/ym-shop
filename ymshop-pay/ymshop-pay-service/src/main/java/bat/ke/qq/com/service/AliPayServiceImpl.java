...
import bat.ke.qq.com.common.constants.Constants;
import bat.ke.qq.com.common.constants.PayAlipayType;
import bat.ke.qq.com.common.utils.AliPayConfig;
import bat.ke.qq.com.common.utils.ConfigUtilZfb;
...
public class AliPayServiceImpl implements IAliPayService {
    ...
    */
   @Override
   public String aliPayQrCode(PayOrder payOrder) {
      logger.info("支付宝扫码支付下单");
      AlipayTradePrecreateRequest alipayRequest = new AlipayTradePrecreateRequest();
      alipayRequest.setReturnUrl(ConfigUtilZfb.returnUrl);//前台通知
      alipayRequest.setNotifyUrl(ConfigUtilZfb.notifyurl);//后台回调
      JSONObject bizContent = new JSONObject();
      bizContent.put("out_trade_no", payOrder.getOutTradeNo());
      bizContent.put("total_amount", payOrder.getTotalFee());//订单金额
      bizContent.put("subject", payOrder.getSubject());//订单标题
      bizContent.put("seller_id", Configs.getPid());//实际收款账号，一般填写商户PID即可
      ...
      try {
         response = AliPayConfig.getAlipayClient().execute(alipayRequest);
         if (response==null || !response.getCode().equals("10000") ){
            if(logger.isErrorEnabled()){
               logger.error("调用支付宝扫码支付接口失败 response ："+response.getBody());
            }
            return null;
         }
      } catch (AlipayApiException e) {
